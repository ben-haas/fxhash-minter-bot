...
export function getTzktData(data, level) {
  if (data) {
    if (params.logBlocks == true) {
      logData(data, level);
    }

    const mintedTokens = getTokenList();
    let mintCount = 0;
    ...
    data.forEach((i) => {
      ...
      if (entrypoint == 'mint_issuer' && amount > 0) {
        ...
        artists.forEach((a) => {
          if (
            ...
            if (a.maxCost >= amount || a.maxCost == 0) {
              console.log(
                `Attempting to mint ${a.alias} GENTK in ${
                  params.mintDelay / 1000
                } seconds........................................`
              );
              params.pKey.forEach((k) => {
                if (mintCount < a.mintAmount || a.mintAmount == 0) {
                  setTimeout(() => {
                    mintGENTK(id, amount, a.gas, k);
                  }, params.mintDelay);
                  mintCount++;
                } else {
                  console.log(
                    `Only minting ${a.mintAmount} GENTK from ${a.alias}`
                  );
                }
              });
            } else {
              console.log(
                `Not minting ${a.alias} GENTK. Max mint price of ${a.maxCost} has been exceeded........................................`
              );
              logErrors(id, 'Limit', 'Cost limit exceeded', 'Not Submitted');
            }
          } else if (sender == a.address && onMintedList == true) {
            console.log(
              `Not minting ${a.alias}, the token has already been minted by the bot.`
            );
            logErrors(id, senderName, 'Token already minted', 'Not Submitted');
          } else if (sender == a.address && timelock > 0) {
            console.log(
              `Not minting ${a.alias}, the token is timelocked for ${timelock} hours.`
            );
          ...
          if (sender == a.address && enabled && onMintedList == false) {
            if (a.maxCost >= amount || a.maxCost == 0) {
              ...
              params.pKey.forEach((k) => {
                if (mintCount < a.mintAmount || a.mintAmount == 0) {
                  setTimeout(() => {
                    mintGENTK(updateId, amount, a.gas, k);
                  }, params.mintDelay);
                  mintCount++;
                } else {
                  console.log(
        ...
        enabled &&
        timelock == '0' &&
        params.freeMints == true
      ) {
        console.log(
          `Attempting to mint GENTK because it's free........................................`
        );
