...
export async function logMints(hash) {
  ...
  await fetch(`https://api.tzkt.io/v1/operations/${hash}`)
    .then((res) => res.json())
    .then((res) => {
      let log = [];
      let status = 'unknown';
...
export function logErrors(id, name, msg, sender) {
  let log = [];
...
export function getTokenList() {
  ...
  if (fs.existsSync(path)) {
    let csv = fs.readFileSync(path, { encoding: 'utf-8' }, function (err) {
      console.log(err);
    });
    let result = [];
...
export function logData(data, block) {
